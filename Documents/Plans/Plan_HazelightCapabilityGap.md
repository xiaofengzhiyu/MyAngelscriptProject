...
- [ ] **P1.2** 固定工程拆分差异，不再把结构差异误写成功能缺口
  - 问题：Hazelight 基线采用 `AngelscriptCode` + `AngelscriptEditor` + `AngelscriptLoader`，并额外拆出 `AngelscriptGAS`、`AngelscriptEnhancedInput`；当前插件已采用 `AngelscriptRuntime` + `AngelscriptEditor` + `AngelscriptTest`，启动收口内聚到 Runtime 的 `UAngelscriptEngineSubsystem`，GAS / EnhancedInput 仍作为主插件依赖。
