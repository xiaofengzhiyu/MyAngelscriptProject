...
- **类 1：已被 `Bind_*.cpp` 手工 lambda 接管**。典型例子是 `AngelscriptWorldLibrary.h`：`Bind_UWorld.cpp:79-82` 用 `UWorld_.Method("TArray<ULevelStreaming> GetStreamingLevels() const", [...] { return UAngelscriptWorldLibrary::GetStreamingLevels(World); })` 显式注册成员方法，AS 脚本里 `World.GetStreamingLevels()` 已经能 work。重启 ScriptMixin 后会被 `Bind_BlueprintCallable.cpp:62-70` 的 `IsScriptDeclarationAlreadyBound` 静默跳过（如签名一致），或者注册成第二条签名引发重载歧义（如签名细微漂移，例如 `TArray<ULevelStreaming@>` vs `TArray<ULevelStreaming>`、`const` 推导）。手工 lambda 的真正价值是**精确签名控制**——去掉冗余指针 `@`、强制 `const`、自定义返回类型映射。这类文件重启 ScriptMixin **没有功能价值**。
