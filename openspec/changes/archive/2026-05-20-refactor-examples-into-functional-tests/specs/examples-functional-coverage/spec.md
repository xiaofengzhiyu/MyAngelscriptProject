...
- Target test layer: UE Functional under `Plugins/Angelscript/Source/AngelscriptTest/Functional/**`, with neighboring established theme directories allowed when they already own the behavior.
- Expected Automation prefix: `Angelscript.TestModule.Functional.<Theme>.*` for functional-theme additions, or existing theme-owned prefixes such as `Angelscript.TestModule.Actor.*`, `Angelscript.TestModule.Component.*`, `Angelscript.TestModule.Delegate.*`, and `Angelscript.TestModule.Widget.*` when extending those families.
- Recommended helper/harness: `Shared/AngelscriptFunctionalTestUtils.h`, `Shared/AngelscriptTestWorld.h`, existing `Functional/<Theme>` helpers, reflective property helpers already used by actor/component/widget/property functional tests, and `AngelscriptTestEngineHelper.*` only for compile-oriented classification checks.
- Verification entry points:
  - `powershell.exe -NoProfile -ExecutionPolicy Bypass -File Tools\RunTests.ps1 -TestPrefix "Angelscript.TestModule.Functional." -Label examples-functional -TimeoutMs 900000`
  - `powershell.exe -NoProfile -ExecutionPolicy Bypass -File Tools\RunTestSuite.ps1 -Suite FunctionalSamples -LabelPrefix examples-functional-samples -TimeoutMs 900000`
  - `powershell.exe -NoProfile -ExecutionPolicy Bypass -File Tools\RunTests.ps1 -Group AngelscriptFunctional -Label examples-functional-group -TimeoutMs 900000`
  - `powershell.exe -NoProfile -ExecutionPolicy Bypass -File Tools\RunBuild.ps1 -Label examples-functional -TimeoutMs 180000`
